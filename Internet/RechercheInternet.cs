...
namespace CollectionDeFilms.Internet
{
    public class RechercheInternet
    {
        ...
        private async Task<InfosFilm> chargePage(string requete)
        {
            ...
            info._realisateur = await cumuleExtract(doc, xpathRealisateur); MainForm.WriteMessageToConsole($"Réalisateur: {info._realisateur}");
            info._acteurs = await cumuleExtract(doc, xpathActeurs); MainForm.WriteMessageToConsole("Acteurs: " + info._acteurs);
            info._genres = await cumuleExtract(doc, xpathGenres); MainForm.WriteMessageToConsole("Genres: " + info._genres);
            info._nationalite = await cumuleExtract(doc, xpathNationalite); MainForm.WriteMessageToConsole("Nationalite: " + info._nationalite);
            info._resume = await cumuleExtract(doc, xpathResume); MainForm.WriteMessageToConsole("Résumé: " + info._resume);
            info._dateSortie = await cumuleExtract(doc, xpathDateSortie); MainForm.WriteMessageToConsole("Date sortie: " + info._dateSortie);

            string imglink = await cumuleExtract(doc, xpathAffiche); MainForm.WriteMessageToConsole("Affiche: " + imglink);
            if (imglink != null)
        ...
        private async Task<string> cumuleExtract(HtmlDocument doc, string xpath)
        {
            List<string> elements = await extract(doc, xpath);
            if (elements == null)
                return "";
        ...
        /// <returns></returns>
        async private Task<List<string>> extract(string requete, string xpath)
        {
            MainForm.WriteMessageToConsole("Requete " + requete);
            HtmlDocument doc = await InternetUtils.loadPage(requete);
            if (doc == null)
            ...
            return await extract(doc, xpath);
        }
        ...
        private async Task<List<string>> extract(HtmlDocument doc, string xpath)
        {
