...
{
    public partial class OldConfigurationDlg : Form
    {
        public OldConfigurationDlg()
        {
            InitializeComponent();
        }
        ...
        {
            EditeRechercheSurSite dlg = new EditeRechercheSurSite();
            if (dlg.ShowDialog(this) == DialogResult.OK)
            {
                RechercheSurSite r = dlg.rechercheInternet;
                if (r != null)
                {
        ...
        private void buttonDeleteClick(object sender, EventArgs e)
        {
            ...
            if (selected > 0)
            {
                ...
                if (nom == null)
                    return;
                if (MessageBox.Show($"Voulez-vous vraiment supprimer la recherche \"{nom}\"?", "Suppression", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
        ...
        private void onClickButtonPrev(object sender, EventArgs e)
        {
            ...
            if (selected > 0)
            {
                ...
                {
                    liste[i].rang = i;
                    if (liste[i] is RechercheSurSite)
                    {
                        BaseConfiguration.instance.updateRechercheInternet((RechercheSurSite)liste[i]);
                    }
                }
                remplitListeRecherches();
